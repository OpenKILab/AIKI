...
tmux kill-session -t vllm-jina 2>/dev/null
...
for ((i=0; i<$NUM_INSTANCES; i++)); do
    ...
    tmux send-keys -t "$SESSION_NAME:vllm-$PORT" "CUDA_VISIBLE_DEVICES=$SELECTED_GPU HF_ENDPOINT=https://hf-mirror.com python3 -m vllm.entrypoints.openai.api_server \
        --model /fs-computility/ai-shen/shared/hf-hub/jinaai/ReaderLM-v2 \
        --trust-remote-code \
        --tensor-parallel-size $TP_SIZE \
        --served-model-name ReaderLM-v2 \
        --dtype auto \
        --gpu-memory-utilization 0.85 \
        --port $PORT \
        --use-v2-block-manager \
        --tokenizer /fs-computility/ai-shen/shared/hf-hub/jinaai/ReaderLM-v2 \
